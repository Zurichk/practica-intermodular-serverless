...
Crea una tabla en DynamoDB *a través de la consola de AWS*. Utiliza *capacidad a demanda* (opción por defecto) y configura la *clave primaria* elegida en el punto anterior.

Como nombre de la tabla, utiliza ~inter-serverless-manual-TUS_INICIALES~, sustituyendo ~TUS_INICIALES~ por el valor correspondiente.

** Creación y despliegue de las funciones lambda del backend
Como puedes comprobar, en el repositorio hay una carpeta denominada ~proyecto~. Dentro de ella hay una estructura de carpetas para una *aplicación SAM*, que nos permitirá realizar el *despliegue del backend* de manera sencilla, sin tener que subir el código de manera manual.

Se proporcionan *dos funciones ya creadas*. Puedes ver su código dentro de la carpeta ~handlers~. También puedes echar un vistazo a la plantilla de SAM, ~template.yml~.
...
Indica un nombre para el stack que incluya tu *nombre y apellidos*. Al finalizar, comprueba que se han creado dos funciones accediendo al *servicio Lambda* en la consola de AWS.
...
- ~putNote~ - Actualizará una nota específica de un usuario determinado. Necesitará el ~noteId~ y el usuario autenticado, así como los datos actualizados de la nota. Esta función es *prácticamente igual* a la función ~postNotes~ que se proporciona.
