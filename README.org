...
Como puedes comprobar, en el repositorio hay una carpeta denominada ~proyecto~. Dentro de ella hay una estructura de carpetas para una *aplicación SAM*. Sin embargo, para empezar, *no utilizaremos SAM para el despliegue*. Haremos un *despliegue manual*, subiendo el código directamente a la consola. Más adelante realizaremos el despliegue mediante SAM.
